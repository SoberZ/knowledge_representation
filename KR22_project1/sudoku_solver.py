#
# Names: Silver Lee-A-Fong, Dexter Roelofsen
# Course: Knowledge Representation
# Description: This file contains a sudoku solver using
# DPLL and three different heuristics: Random Choice,
# JW-OS and MOMS.
#
...
def single_BCP(clauses, unit):
    """
    Perform Boolean Clause Propagation on a CNF
    with one specific unit clause.
    """
    new_clauses = []
    for clause in clauses:
...
def evaluate_expression(clauses):
    """
    Check if there's an empty clause in the CNF.
    """
    for clause in clauses:
        if not len(clause):
            return False
